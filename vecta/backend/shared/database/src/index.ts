...
//   psql $DATABASE_URL -f backend/shared/database/migrations/005_vecta_ledger.sql
//   psql $DATABASE_URL -f backend/shared/database/migrations/006_vecta_insurance.sql
//
// Or run all migrations in sequence (from vecta/ repo root):
//   for f in backend/shared/database/migrations/*.sql; do psql $DATABASE_URL -f "$f"; done
//
...
// ---------------------------------------------------------------------------
// Run via: psql $DATABASE_URL -f backend/shared/database/migrations/002_plaid_connections.sql
