...
router.post('/roommate/profile', async (req: Request, res: Response) => {
  try {
    ...
    const aiResponse = await fetch(
      `${process.env.COMPLIANCE_AI_URL ?? 'http://compliance-ai:3007'}/housing/roommate-profile`,
      {
        method: 'POST',
...
router.get('/roommate/matches', async (req: Request, res: Response) => {
  try {
    ...
    const aiResponse = await fetch(
      `${process.env.COMPLIANCE_AI_URL ?? 'http://compliance-ai:3007'}/housing/roommate-matches/${studentId}`,
    );
