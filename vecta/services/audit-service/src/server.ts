...
import { hmacSign, hmacVerify } from '@vecta/crypto';
...
function requireInternalAuth(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!sig || !ts) {
    res.status(401).json({ error: 'MISSING_INTERNAL_AUTH' });
    return;
  }

  // Reject stale requests (replay protection — 30s window)
  const age = Date.now() - parseInt(ts, 10);
  if (Math.abs(age) > 30_000) {
    res.status(401).json({ error: 'TIMESTAMP_EXPIRED' });
    return;
  }

  const payload = `${ts}:${JSON.stringify(req.body)}`;
  if (!hmacVerify(payload, sig, INTERNAL_SECRET)) {
