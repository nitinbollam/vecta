...
import { sendLandlordUpgradeEmail } from './email.service';

const logger = createLogger('checkr-service');
...
async function checkrGet<T>(path: string): Promise<T> {
  ...
  if (!res.ok) throw new Error(`Checkr GET ${path} failed: ${res.status}`);
  return res.json() as Promise<T>;
}
...
export async function initiateBackgroundCheck(landlordId: string): Promise<{
  candidateId:   string;
  reportId:      string;
  consentUrl:    string;
  ...
  const landlord = await queryOne<{
    ...
    'SELECT id, email, full_name FROM landlord_profiles WHERE id = $1',
    [landlordId],
  );
  ...
  const candidate = await checkrPost<{
  ...
  const report = await checkrPost<{
    id: string; status: string; consent_link?: string;
  }>('/reports', {
  ...
  await query(
    `UPDATE landlord_profiles
     SET background_check_provider = 'Checkr',
         ...
         updated_at                = NOW()
     WHERE id = $1`,
    [landlordId, report.id],
  );

  logger.info(
    { landlordId, reportId: report.id, candidateId: candidate.id },
...
// ---------------------------------------------------------------------------
// Webhook handler — called when Checkr reports completion
// ---------------------------------------------------------------------------
...
export async function handleCheckrWebhook(
  ...
  if (!type.startsWith('report.')) return; // Only handle report events
  ...
  const newStatus: 'APPROVED' | 'REJECTED' =
    adjudication === 'engaged' ? 'APPROVED' : 'REJECTED';
  ...
  logger.info(
    { landlordId: updated.id, reportId: report.id, newStatus },
    'Background check completed',
  );

  // Notify landlord if approved → TRUSTED tier unlocked
  if (newStatus === 'APPROVED') {
    const emailTo: { toEmail: string; toName?: string } = { toEmail: updated.email };
    if (updated.full_name) emailTo.toName = updated.full_name;
    await sendLandlordUpgradeEmail(emailTo);
  }
}
...
export async function getBackgroundCheckStatus(landlordId: string): Promise<{
  ...
  const landlord = await queryOne<{
    ...
    'SELECT background_check_id, background_check_status FROM landlord_profiles WHERE id = $1',
    [landlordId],
  );
  ...
  const status = (landlord.background_check_status ?? 'PENDING') as
    'NOT_STARTED' | 'PENDING' | 'APPROVED' | 'REJECTED';

  if (status === 'PENDING') {
    // Fetch live status from Checkr (for accurate progress)
    try {
      const report = await checkrGet<{ status: string; estimated_completion_time?: string }>(
        `/reports/${landlord.background_check_id}`,
      );
      const est = report.estimated_completion_time;
      return est !== undefined
        ? { status: 'PENDING' as const, estimatedCompletion: est }
