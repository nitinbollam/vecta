...
exports.sendLandlordVerifyEmail = sendLandlordVerifyEmail;
exports.sendLandlordUpgradeEmail = sendLandlordUpgradeEmail;
exports.sendStudentTokenUsedEmail = sendStudentTokenUsedEmail;
exports.sendKycApprovedEmail = sendKycApprovedEmail;
exports.sendKycRejectedEmail = sendKycRejectedEmail;
exports.sendLocReadyEmail = sendLocReadyEmail;
const logger_1 = require("@vecta/logger");
const logger = (0, logger_1.createLogger)('email-service');
// ---------------------------------------------------------------------------
// Provider config
// ---------------------------------------------------------------------------
...
async function sendViaProvider(payload) {
    ...
    logger.info({ to: payload.personalizations[0]?.to[0]?.email, template: payload.template_id }, 'Email sent');
}
// ---------------------------------------------------------------------------
 ...
 */
async function sendLandlordVerifyEmail(params) {
 ...
 */
async function sendLandlordUpgradeEmail(params) {
    await sendViaProvider({
        from: { email: FROM_EMAIL, name: FROM_NAME },
        personalizations: [{
                to: [{ email: params.toEmail, ...(params.toName != null ? { name: params.toName } : {}) }],
 ...
 */
async function sendStudentTokenUsedEmail(params) {
    await sendViaProvider({
        from: { email: FROM_EMAIL, name: FROM_NAME },
        personalizations: [{
 ...
 */
async function sendKycApprovedEmail(params) {
    await sendViaProvider({
        from: { email: FROM_EMAIL, name: FROM_NAME },
        personalizations: [{
                to: [{ email: params.toEmail, name: params.studentName }],
 ...
 */
async function sendKycRejectedEmail(params) {
    await sendViaProvider({
        from: { email: FROM_EMAIL, name: FROM_NAME },
        personalizations: [{
                to: [{ email: params.toEmail, name: params.studentName }],
 ...
 */
async function sendLocReadyEmail(params) {
    await sendViaProvider({
        from: { email: FROM_EMAIL, name: FROM_NAME },
        personalizations: [{
                to: [{ email: params.toEmail, name: params.studentName }],
