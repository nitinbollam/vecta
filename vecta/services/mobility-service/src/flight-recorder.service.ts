...
import { Pool } from "pg";
import type { FlightRecorderEntry, VehicleLeaseConsent, StudentRole } from "@vecta/types";
import { createLogger } from "@vecta/logger";
import { getPool } from "@vecta/database";

const logger = createLogger("mobility-flight-recorder");
...
export class ScheduleEValidator {
  ...
  async validateRideCompliance(params: {
    ...
    rideId: string;
  }): Promise<{ valid: true; lessorStudentId: string; rentalIncomePct: number }> {
    // 1. Fetch the active lease for this vehicle
    const leaseResult = await this.db.query(
      `SELECT
    ...
    if (leaseResult.rows.length === 0) {
      throw new F1ComplianceError(
        `Vehicle ${params.vehicleVin} is not enrolled in the Vecta fleet. Cannot log ride.`
      );
    }
    ...
    if (lessorStudentId === params.proposedDriverId) {
      ...
      throw new F1ComplianceError(
        `CRITICAL F-1 COMPLIANCE VIOLATION PREVENTED: Student ${lessorStudentId} ` +
    ...
    if (!lessorRoles.includes("LESSOR")) {
      throw new F1ComplianceError(
        `Student ${lessorStudentId} has an active lease but LESSOR role not assigned. ` +
    ...
    const driverCheck = await this.db.query(
      "SELECT roles FROM students WHERE id = $1",
      [params.proposedDriverId]
    );

    if (driverCheck.rows.length > 0) {
      const driverRoles: StudentRole[] = driverCheck.rows[0].roles;
      if (driverRoles.includes("LESSOR" as StudentRole)) {
        throw new F1ComplianceError(
          `Driver ${params.proposedDriverId} is also a Vecta LESSOR. ` +
    ...
    const rentalIncomePct = parseFloat(
      process.env.LESSOR_RENTAL_INCOME_PCT ?? "0.30"  // 30% to lessor by default
    );
...
export class FlightRecorderService {
  private readonly scheduleEValidator: ScheduleEValidator;
  ...
  async logCompletedRide(params: {
    ...
    const { lessorStudentId, rentalIncomePct } =
      await this.scheduleEValidator.validateRideCompliance({
        vehicleVin: params.vehicleVin,
    ...
    const prevBlock = await this.db.query(
      `SELECT block_index, crypto_hash FROM flight_recorder
       WHERE vehicle_vin = $1
  ...
  async exportAuditChain(params: {
    ...
    const records = await this.db.query(
      `SELECT * FROM flight_recorder
       WHERE lessor_student_id = $1
         AND EXTRACT(YEAR FROM start_ts) = $2
       ORDER BY block_index ASC`,
      [params.lessorStudentId, params.taxYear]
    );

    const entries = records.rows.map((r): FlightRecorderEntry => ({
      id: r.id,
      rideId: r.ride_id,
    ...
    const totalRentalIncomeCents = entries.reduce(
      (sum, e) => sum + e.rentalIncomeCents,
      0
    );
    ...
    if (chainIntegrity === 'VERIFIED' && entries.length > 0) {
      logger.info(
        { lessorStudentId: params.lessorStudentId, taxYear: params.taxYear },
        'Chain export verified — anchor via audit-service job or shared package in production',
      );
    }
  ...
  private verifyChainIntegrity(entries: FlightRecorderEntry[]): "VERIFIED" | "COMPROMISED" {
...
export class DSOComplianceMemoService {
  constructor(private readonly db: Pool) {}

  async generateMemo(studentId: string): Promise<{
    memoText: string;
    memoHtml: string;
    generatedAt: string;
  }> {
...
`.trim();
    ...
    await this.db.query(
      ...
      [studentId, "DSO_MEMO_GENERATED", JSON.stringify({ generatedAt, totalIncomeUSD })]
    );
...
export class VehicleEnrollmentService {
  constructor(private readonly db: Pool) {}
  ...
  async enrollVehicleWithConsent(params: {
    ...
    vehicleModel: string;
    vehicleYear: number;
    // Consent from UI (all 4 must be TRUE — schema enforces this)
    ...
    // Request metadata for legal record
    consentIpAddress: string;
    consentUserAgent: string;
    tosVersion: string;
  }): Promise<{ leaseId: string; leaseActive: boolean }> {
    // Validate all consents are explicitly TRUE
    const allConsentsGiven = (
      params.strictlyPassiveAcknowledged &&
    ...
    if (!allConsentsGiven) {
      throw new F1ComplianceError(
        "All four compliance clauses must be acknowledged before vehicle enrollment. " +
        "Missing: " + [
    ...
    const consentData: VehicleLeaseConsent = {
    ...
    consentData.signatureHash = crypto
      .createHash("sha256")
...
const _mobilityPool = getPool();

export const flightRecorderService = new FlightRecorderService(_mobilityPool);
export const dsoComplianceMemoService = new DSOComplianceMemoService(_mobilityPool);
export const vehicleEnrollmentService = new VehicleEnrollmentService(_mobilityPool);
