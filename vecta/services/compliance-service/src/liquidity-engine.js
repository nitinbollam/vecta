...
exports.checkLiquidityEligibility = checkLiquidityEligibility;
exports.allocateLiquidity = allocateLiquidity;
exports.seedVectaPool = seedVectaPool;
exports.addUniversityPool = addUniversityPool;
exports.addCorporatePartner = addCorporatePartner;
exports.getPoolStats = getPoolStats;
const database_1 = require("@vecta/database");
const logger_1 = require("@vecta/logger");
const logger = (0, logger_1.createLogger)('liquidity-engine');
// ---------------------------------------------------------------------------
// Eligibility check — which strategy applies?
// ---------------------------------------------------------------------------
async function checkLiquidityEligibility(params) {
    // Reject if student already has an active allocation
    const existing = await (0, database_1.queryOne)(`SELECT id FROM liquidity_allocations
     WHERE student_id = $1 AND status = 'ACTIVE' AND expires_at > NOW()`, [params.studentId]);
    if (existing) {
        return {
    ...
    const uniPool = await (0, database_1.queryOne)(`SELECT id, total_capacity_usd, deployed_usd
     FROM liquidity_pool
     ...
     LIMIT 1`, [params.universityName, params.city]);
    if (uniPool) {
    ...
    const corpPool = await (0, database_1.queryOne)(`SELECT id, sponsor_name
     FROM liquidity_pool
    ...
    const vectaPool = await (0, database_1.queryOne)(`SELECT id, total_capacity_usd, deployed_usd, reserve_ratio
     FROM liquidity_pool
...
// ---------------------------------------------------------------------------
async function allocateLiquidity(params) {
    return (0, database_1.withTransaction)(async (client) => {
        // Lock the pool row to prevent concurrent over-allocation
        const pool = await client.query(`SELECT id, total_capacity_usd, deployed_usd, reserve_ratio, pool_type, sponsor_name
       FROM liquidity_pool WHERE id = $1 AND active = TRUE FOR UPDATE`, [params.poolId]);
        if (pool.rowCount === 0)
        ...
        const alloc = await client.query(`INSERT INTO liquidity_allocations
         (pool_id, student_id, lease_application_id, allocated_usd, months_covered, expires_at)
       VALUES ($1,$2,$3,$4,$5,$6)
       RETURNING id`, [params.poolId, params.studentId, params.leaseApplicationId, allocAmount, params.monthsCovered, expiresAt]);
        // Deduct from pool
        if (allocAmount > 0) {
            await client.query(`UPDATE liquidity_pool SET deployed_usd = deployed_usd + $2, updated_at = NOW() WHERE id = $1`, [params.poolId, allocAmount]);
        }
        const allocationId = alloc.rows[0].id;
        (0, logger_1.logAuditEvent)('LIQUIDITY_ALLOCATED', params.studentId, allocationId, {
            strategy: params.strategy,
            poolId: params.poolId,
            amountUsd: allocAmount,
            expiresAt: expiresAt.toISOString(),
        });
        logger.info({ studentId: params.studentId, strategy: params.strategy, allocAmount }, 'Liquidity allocated');
        return {
            allocationId,
            poolId: params.poolId,
            strategy: params.strategy,
...
// ---------------------------------------------------------------------------
async function seedVectaPool(params) {
    const result = await (0, database_1.queryOne)(`INSERT INTO liquidity_pool
     ...
     RETURNING id`, [params.totalCapacityUsd, params.targetCity ?? null]);
    logger.info({ poolId: result.id, capacity: params.totalCapacityUsd, city: params.targetCity }, 'Vecta guaranteed rent pool seeded');
    return result.id;
}
async function addUniversityPool(params) {
    const result = await (0, database_1.queryOne)(`INSERT INTO liquidity_pool
     ...
     RETURNING id`, [params.universityName, params.capacityUsd, params.universityName, params.city]);
    logger.info({ poolId: result.id, university: params.universityName, capacity: params.capacityUsd }, 'University-backed pool added');
    return result.id;
}
async function addCorporatePartner(params) {
    const result = await (0, database_1.queryOne)(`INSERT INTO liquidity_pool
       (pool_type, sponsor_name, sponsor_type, total_capacity_usd, target_city, reserve_ratio)
     VALUES ('CORPORATE_PARTNER',$1,'CORPORATE',9999999,$2,0.00)
     RETURNING id`, [params.partnerName, params.city]);
