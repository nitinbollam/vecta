...
exports.getSocialProofStats = getSocialProofStats;
exports.getComparableReport = getComparableReport;
exports.onboardLandlord = onboardLandlord;
exports.recordAcceptance = recordAcceptance;
exports.recordUniversityIntegration = recordUniversityIntegration;
const database_1 = require("@vecta/database");
const logger_1 = require("@vecta/logger");
const logger = (0, logger_1.createLogger)('landlord-credibility');
async function getSocialProofStats(city, state) {
    // Total landlords who have accepted at least one certificate
    const totalLandlordsResult = await (0, database_1.queryOne)(`SELECT COUNT(DISTINCT landlord_id)::text AS count
     FROM trust_signal_events
     WHERE event_type = 'CERTIFICATE_ACCEPTED'`);
    // Total students placed
    const totalStudentsResult = await (0, database_1.queryOne)(`SELECT COUNT(*)::text AS count
     FROM lease_applications
     WHERE status IN ('SIGNED','PENDING_SIGNATURE')`);
    // Acceptance rate
    const certsIssuedResult = await (0, database_1.queryOne)(`SELECT
    ...
    // Avg decision time
    const avgDecisionResult = await (0, database_1.queryOne)(`SELECT AVG(EXTRACT(EPOCH FROM (la.created_at - ttc.created_at)))::int::text AS avg_seconds
     FROM lease_applications la
     JOIN tenant_trust_certificates ttc ON ttc.cert_id = la.cert_id`);
    // Cities served
    const citiesResult = await (0, database_1.queryOne)(`SELECT COUNT(DISTINCT city)::text AS count
     FROM trust_signal_events
     WHERE city IS NOT NULL`);
    // University partners
    const uniResult = await (0, database_1.queryOne)(`SELECT COUNT(DISTINCT university_name)::text AS count
     FROM trust_signal_events
     WHERE event_type = 'UNIVERSITY_INTEGRATION'`);
    // Recent acceptances (anonymised)
    const recentResult = await (0, database_1.query)(`SELECT city, state, university_name, created_at
     FROM trust_signal_events
     WHERE event_type = 'CERTIFICATE_ACCEPTED'
    ...
    return {
        ...
        recentAcceptances: recentResult.rows.map((r) => ({
            ...
            daysAgo: Math.floor((Date.now() - new Date(r.created_at).getTime()) / (1000 * 60 * 60 * 24)),
        })),
    };
}
async function getComparableReport(zipCode, guaranteeTier) {
...
async function getCityCount() {
    const r = await (0, database_1.queryOne)(`SELECT COUNT(DISTINCT city)::text AS count FROM trust_signal_events WHERE city IS NOT NULL`);
    return parseInt(r?.count ?? '0', 10);
}
// ---------------------------------------------------------------------------
// Landlord onboarding — converts a first-viewer into a network member
// ---------------------------------------------------------------------------
async function onboardLandlord(params) {
    // Find referrer if any
    let referredBy = null;
    if (params.referralCode) {
        const referrer = await (0, database_1.queryOne)('SELECT id FROM landlord_network WHERE referral_code = $1', [params.referralCode]);
        referredBy = referrer?.id ?? null;
        if (referredBy) {
            await (0, database_1.query)('UPDATE landlord_network SET referred_count = referred_count + 1 WHERE id = $1', [referredBy]);
        }
    }
    // Generate unique referral code for this landlord
    const refCode = `VL-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
    const result = await (0, database_1.queryOne)(`INSERT INTO landlord_network
    ...
    logger.info({ networkId: result.id, cities: params.cities }, 'Landlord onboarded to network');
...
async function recordAcceptance(params) {
    await (0, database_1.query)(`INSERT INTO trust_signal_events
    ...
    await (0, database_1.query)(`UPDATE landlord_network
...
//# sourceMappingURL=landlord-credibility.service.js.map